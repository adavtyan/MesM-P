...
FixPrintDebugCoords::FixPrintDebugCoords(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg)
{
  ...
  int iarg = 5;
  while (iarg < narg) {
    if (strcmp(arg[iarg],"no_xyz") == 0) x_flag = 0;
    else if (strcmp(arg[iarg],"vel") == 0) v_flag = 1;
    else if (strcmp(arg[iarg],"quat") == 0) quat_flag = 1;
    else if (strcmp(arg[iarg],"angmom") == 0) angmom_flag = 1;
    else if (strcmp(arg[iarg],"phi") == 0) phi_flag = 1;
  ...
  int len = strlen("index type X Y Z VX VY VZ QW QX QY QZ OX OY OZ PHIM PHIB") + 1;
  header = new char[len];
  strcpy(header, "index type");
  if (x_flag) strcat(header, " X Y Z");
  if (v_flag) strcat(header, " VX VY VZ");
  if (quat_flag) strcat(header, " QW QX QY QZ");
  if (angmom_flag) strcat(header, " OX OY OZ");
  if (phi_flag) strcat(header, " PHIM PHIB");
...
FixPrintDebugCoords::~FixPrintDebugCoords()
{
...
int FixPrintDebugCoords::setmask()
{
  int mask = 0;
  mask |= END_OF_STEP;
  mask |= POST_INTEGRATE;
...
void FixPrintDebugCoords::init()
{
  avec = (AtomVecEM2 *) atom->style_match("em2");
  if (!avec) error->all(FLERR,"Fix print_debug_coords requires atom style em2");
}

/* ---------------------------------------------------------------------- */

void FixPrintDebugCoords::end_of_step()
{
}

/* ---------------------------------------------------------------------- */

void FixPrintDebugCoords::post_integrate()
{
