...
FixNHEM2ROT::FixNHEM2ROT(LAMMPS *lmp, int narg, char **arg) :
  FixNH(lmp, narg-1, arg)
{
  if (narg < 5) error->all(FLERR,"Illegal fix nvt/npt/nph em2/rot command");
...
void FixNHEM2ROT::read_conf_file(char *filename)
{
  int i;
  int ilo,ihi,count;
...
inline void FixNHEM2ROT::print_log(char *line)
{
  if (screen) fprintf(screen, line);
  if (logfile) fprintf(logfile, line);
}

char *FixNHEM2ROT::ltrim(char *s)
{     
  while(isspace(*s)) s++;     
  return s; 
}  

char *FixNHEM2ROT::rtrim(char *s)
{
  char* back;
  int len = strlen(s);
...
char *FixNHEM2ROT::trim(char *s)
{     
  return rtrim(ltrim(s));  
}

bool FixNHEM2ROT::isEmptyString(char *str)
{
  int len = strlen(str);
